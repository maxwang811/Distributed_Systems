...
function query(indexFile, args) {
  const raw = args.join(' ');
  let processed = execSync(`echo "${raw + '\n'}" | ./c/process.sh | ./c/stem.js`, {encoding: 'utf-8'});
  processed = processed.trim();
  const indexData = fs.readFileSync(indexFile, 'utf-8');
  const lines = indexData.split(/\r?\n/);
  for (const line of lines) {
    // console.log(line);
    const parts = line.split('|');
    if (parts[0].trim().includes(processed)) { // Prevent matching w/ things in the URL
      console.log(line);
    }
  }
}
