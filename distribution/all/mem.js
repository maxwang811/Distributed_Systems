...
function mem(config) {
  ...
  function reconf(configuration, callback) {
    callback = typeof callback === 'function' ? callback : () => {};
    return reconfigure('mem', context, configuration, callback);
  }
...
function listKeys(gid, callback) {
  ...
  if (!groupComm || typeof groupComm.send !== 'function') {
    callback(new Error(`mem.get: comm unavailable for group "${gid}"`));
    return;
  }

  groupComm.send([{gid, key: null}], {service: 'mem', method: 'get'}, (errors, values) => {
...
function pickNodeForKey(gid, key, group, hasher) {
  ...
  if (nodes.length === 0) {
    throw new Error(`mem: group "${gid}" has no members`);
  }
  ...
  if (!node) {
    throw new Error(`mem: unable to resolve node for key "${key}"`);
  }
  return node;
}
 ...
 * @param {{gid: string, hash: Function}} context
