...
*/
  ...
  const reducer = (key, values) => {
    const out = {};
    out[key] = values.reduce((acc, val) => acc + val, 0);
    return out;
  };
...
test('(10 pts) (scenario) all.mr:strmatch', (done) => {
  ...
  const dataset = [
    {'user-001': 'This should match'},
    {'user-0001': 'This should not match'},
    {'user-01x': 'no'},
    {'store-001': 'no'},
    {'user-159': 'yes'},
  ];

  const expected = ['user-159', 'user-001'];
  ...
  const doMapReduce = () => {
    distribution.strmatch.store.get(null, (e, v) => {
      ...
      distribution.strmatch.mr.exec({keys: v, map: mapper, reduce: reducer}, (e, v) => {
        try { // [ { match: expected } ]
          expect(v[0].match).toEqual(expect.arrayContaining(expected));
