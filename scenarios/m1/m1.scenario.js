...
test('(3 pts) (scenario) expected object', () => {
  ...
  const serializedObject = '{"type":"string","value":"bruno"}'; /* Add here the expected serialized string by using util.serialize */
...
/* Come up with an object that uses all valid (serializable)
  ...
  const func = (x) => x * x;
  const object = {'a': 2, 'b': null, 'c': new Date(), 'd': new Error('x'),
    'e': true, 'f': func, 'g': 'str', 'h': [], 'i': {}, 'j': undefined};
...
/* Come up with a string that is not a valid serialized object. */
  ...
  const malformedSerializedString = '{type: "not ar eal type", "value": 1203129 2}';
